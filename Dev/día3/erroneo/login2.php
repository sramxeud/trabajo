...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!ctype_digit($nombre)) {
        $error_nombre = "La nombre solo puede contener números.";
    }
    
    // Consulta a la tabla 'usuario' para verificar el nombre
    if (empty($error_contra) && empty($error_nombre)) {
        ...
        if (mysqli_num_rows($result) == 0) {
            ...
            // Registrar intento fallido nombre no valido
            ...
            // Actualizar la nueva tabla
            $query = "INSERT INTO nueva_tabla (nombre, ip_pc, nro_i) VALUES ('$nombre', '$ip_pc', 1) ON DUPLICATE KEY UPDATE nro_i = nro_i + 1";
            if (!mysqli_query($link, $query)) {
                echo "Error al actualizar la nueva tabla: " . mysqli_error($link);
            }
        } 
    }
